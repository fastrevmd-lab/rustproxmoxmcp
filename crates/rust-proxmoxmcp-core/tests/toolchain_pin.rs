...
//! kept the two in step, which is the open item on #34: they agree today by
...
fn repo_root() -> PathBuf {
    ...
    path.pop(); // crates/rust-proxmoxmcp-core -> crates
...
fn the_container_builder_matches_the_pinned_toolchain() {
    ...
    assert!(
        ...
        "Dockerfile builds on Rust {builder} but rust-toolchain.toml pins {channel}. \
         ...
         digest — so the image is built with the compiler this repo is tested on (#34)."
    );
}
